...
use App\Paycom\Merchant;
use App\Paycom\Response;
use App\Paycom\PaycomException;
...
class Application
{
    ...
    private function CheckPerformTransaction()
    {
        ...
        if (intval($this->request->params['amount']) < 1000) {
            $this->response->error(
        ...
        //created by ME for testing if there enough money in request  
    ...
    private function CheckTransaction()
    {
        ...
        $this->response->send([
            ...
            'perform_time' => isset($found->perform_time)
             ? 1 * Format::timestamp2milliseconds(Format::datetime2timestamp($found->perform_time)) : 0, 
            //'perform_time' => Format::datetime2timestamp($found->perform_time),
            'cancel_time' => isset($found->cancel_time)
             ?  1 * Format::timestamp2milliseconds(Format::datetime2timestamp($found->cancel_time)) : 0, 
            //'cancel_time' => Format::datetime2timestamp($found->cancel_time),
            'transaction' => (string)$found->id,
            'state' =>  (int)$found->state, //(int)$found->state, // original $found->state
    ...
    private function PerformTransaction()
    {
        ...
        switch ($found->state) {
            ...
            case Transaction::STATE_COMPLETED: // handle complete transaction
                ...
                $this->response->send([
                    'transaction' => (string)$found->id,
                    'perform_time' => (int)Format::datetime2timestamp($found->perform_time)*1000,
                    'state' => (int)$found->state
                ]);
                break;
    ...
    private function CancelTransaction()
    {
        ...
        switch ($found->state) {
            ...
            case Transaction::STATE_CANCELLED_AFTER_COMPLETE:
                $this->response->send([
                    'transaction' => strval($found->id),
                    'cancel_time' => 1 * Format::timestamp2milliseconds(Format::datetime2timestamp($found->cancel_time)),
                    'state' => (int)$found->state
                ]);
                break;
